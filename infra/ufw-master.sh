...
# 7. Allow K3s node-specific communication from Worker (192.168.100.242)
echo "Allowing K3s traffic from worker (192.168.100.242)..."
ufw allow proto tcp from 192.168.100.242 to any port 6443 comment 'Allow k3s agent API access'
ufw allow proto tcp from 192.168.100.242 to any port 10250 comment 'Allow kubelet API access'
ufw allow proto udp from 192.168.100.242 to any port 8472 comment 'Allow Flannel VXLAN'
